...
Client.prototype.runExternal = function (idx, callback, tries) {
  var parameters = this.project.externalFunction(this.population[idx]),
    file       = this.project.externalProgram,
    THIS       = this,
    MAX_TRIES  = 5,
  ...
  ProcessManager.execFile(file, parameters, options,
    function (error, stdout, stderr) {
      if (error) {
        ...
        fitness = parseInt(stdout, 10);
        if (!isNaN(fitness)) {
          THIS.fitness[idx] = fitness;
          callback(null, fitness);
        } else {
          console.error("Fitness NaN " + stdout);
        }
      }
    });
};
...
Client.prototype.crossMattingPool = function (selection, populationSize) {
  var i             = 0,
    ...
    idx2          = 0,
    newChromosome = [];
  for (i = selection.length; i < populationSize; i += 1) {
...
Client.prototype.processJob = function (callback) {
  var THIS           = this,
    ...
    amountMutation = populationSize * this.project.mutationPercent;
  this.fitness       = [];
...
Client.prototype.printReport = function () {
  var workTime                = this.workTimer.getTime(),
    comunicationTimeRequest = this.requestTimer.getTime(),
    comunicationTimeDeliver = this.deliverTimer.getTime();
...
Client.prototype.deliverJob = function () {
  ...
  c = 0;
  for (i = 0; i < this.fitness.length; i += 1) {
...
Client.prototype.requestJob = function () {
  var THIS = this,
    requestOptions = { url  : this.urlServer,
                         form : { action          : 'request',
  ...
  Request.post(requestOptions, function (error, response, body) {
    ...
    var objBody = JSON.parse(body), sleepTime = 1000;
    if (objBody.finalized === undefined) {
      ...
      if (objBody.sleep === undefined) {
        ...
        if (THIS.project !== undefined &&
            THIS.project.sleepTime !== undefined) {
